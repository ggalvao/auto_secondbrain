            ...
            with st.spinner("Uploading vault..."):
                try:
                    ...
                    if response.get("status") == "success":
                        ...
                        st.info("Processing has started. Check the Vault Management page for status.")
                    else:
...
def vault_management_page():
    ...
    try:
        ...
        df = pd.DataFrame([
            {
                ...
                "Created": datetime.fromisoformat(vault["created_at"].replace('Z', '+00:00')).strftime("%Y-%m-%d %H:%M"),
                "ID": vault["id"]
            }
            for vault in vaults
        ])
        ...
        selected_vault_name = st.selectbox(
            "Select vault for details",
            options=list(vault_names.keys())
        )
        ...
        if selected_vault_name:
            ...
            if st.button(f"🗑️ Delete {vault['name']}", type="secondary"):
                if st.confirm(f"Are you sure you want to delete '{vault['name']}'?"):
                    try:
...
def upload_vault(name: str, file) -> Dict[str, Any]:
    """Upload vault to API."""
